...
#    "command": "dga",
#    "args": {
#      "algorithm":   "date_seed",   # date_seed | xor_lcg | wordlist
#      "count":       50,
#      "rate":        1.0,
...
class DGAPlugin(MIATPlugin):
    ...
    async def execute(self, args: dict) -> None:
        ...
        # Generate domains in executor (CPU work)
    ...
    def _gen_xor_lcg(self, count, secret, tld, seed_date):
        domains   = []
        seed_str  = f"{seed_date.isoformat()}:{secret}"
        seed_num  = int.from_bytes(hashlib.md5(seed_str.encode()).digest()[:4], 'big')
        xor_key   = int.from_bytes(hashlib.md5(secret.encode()).digest()[:4], 'big')
    ...
    def _gen_wordlist(self, count, secret, tld, seed_date):
        domains  = []
        seed_str = f"{seed_date.isoformat()}:{secret}"
        seed_int = int.from_bytes(hashlib.sha256(seed_str.encode()).digest(), 'big')
